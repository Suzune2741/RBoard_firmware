...
static int cmd_write(void *buffer, int buffer_size)
{
  ...
  if (irep_write_addr_ == page_top)
    next_page_top = page_top;
...
static int cmd_verify(void)
{
  ...
  // バイトコードの取得(showprogコマンドの実装を参考にした)
  const uint8_t *addr = (const uint8_t *)FLASH_SAVE_ADDR;
  uint8_t data[MRBC_MEMORY_SIZE];
  size_t data_index = 0;
  while (strncmp((const char *)addr, RITE, sizeof(RITE)) == 0)
  {
    uint32_t size = 0;
  ...
  char buf[20];
  mrbc_snprintf(buf, sizeof(buf), "+OK %02x\r\n", crc);
  STRM_PUTS(buf);
  return 0;
}
...
int receive_bytecode(void *buffer, int buffer_size)
{
  ...
  while (1)
  {
    ...
    {
      if (strcmp(token, TBL_COMMANDS[i].command) == 0)
        break;
    }
    ...
    // execute command.
    if ((TBL_COMMANDS[i].function)(buffer, buffer_size) == 1)
      break;
  }
...
void *pickup_task(void *task)
{
  ...
  if (task)
  {
    if (strncmp(task, RITE, sizeof(RITE)) != 0)
      return 0;
